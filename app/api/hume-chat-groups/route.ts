...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `https://api.hume.ai/v0/evi/chat_groups?page_number=${pageNumber}&page_size=${pageSize}&ascending_order=${ascendingOrder}`;
    ...
    console.log('Hume API chat groups response:', data);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error fetching chat groups:', error);
    return NextResponse.json(
      { error: 'Failed to fetch chat groups', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
} 